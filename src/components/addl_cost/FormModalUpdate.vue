<template>
  <TransitionRoot as="template" :show="isModalUpdateOpen">
    <Dialog
      as="div"
        ...
        >
          ...
          >
            ...
            <div class="w-full px-5 bg-blueGray-200">
              <form v-if="addlCost.data" @submit.prevent="update">
                <div class="relative w-full mb-3">
                  ...
                  <input
                    v-model="addlCost.data.category"
                    type="text"
                  ...
                  />
                  <!-- Start of validation employee category -->
                  ...
                  <!-- End of validation employee category -->
                </div>
                <div class="relative w-full mb-3">
                  <label
                    class="block mb-2 text-xs font-semibold uppercase  text-blueGray-600"
                  ...
                  <Listbox v-model="addlCost.data.type">
                ...
                <div class="relative w-full mb-3">
                  ...
                  <textarea
                    v-model="addlCost.data.description"
                    class="w-full text-sm transition-all duration-150 ease-linear bg-white border-0 rounded shadow  placeholder-blueGray-300 text-blueGray-600 focus:outline-none focus:ring"
...
const isModalUpdateOpen = computed(() => {
  return props.isModalUpdateOpen;
});
const emits = defineEmits(["close"]);
const addlCost = computed(() => {
  return store.getters["cost/getStateCost"];
});
...
const update = () => {
  ...
  store
    .dispatch("cost/updateData", {
      id: addlCost.value.data.id,
      project_id: addlCost.value.data.project_id,
      category: addlCost.value.data.category,
      cost: addlCost.value.data.cost,
      description: addlCost.value.data.description,
      type: addlCost.value.data.type,
    })
    .then((res) => {
      store.dispatch("cost/getAllData", addlCost.value.data.project_id);
