<template>
  <TransitionRoot as="template" :show="isModalDeleteOpen">
                ...
                >
                  ...
                  <div class="mt-2">
                    <p class="text-sm text-blueGray-500">
                      {{ modalContent.body }} kategori
                      <span class="font-semibold text-blueGray-800">{{
                        category
                      }}</span>
                      <span> dari biaya lain?</span>
                    </p>
                  </div>
                </div>
              </div>
...
const props = defineProps({
  ...
  contentModalDelete: Object,
  category: String,
});
const emits = defineEmits(["close"]);
const isModalDeleteOpen = computed(() => {
  return props.isModalDeleteOpen;
});
const store = useStore();
const modalContent = computed(() => {
  return props.contentModalDelete;
});
