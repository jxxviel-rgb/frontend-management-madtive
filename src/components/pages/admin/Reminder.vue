          ...
          <div>
            ...
            <div class="flex flex-wrap">
              ...
              <div class="flex items-center px-4">
                <!-- <div
                  class="min-w-0 mb-6 ml-0 mr-8 rounded shadow-lg lg:-ml-6 md:-ml-6"
                >
                  <button
            ...
            >
              <div class="block w-full overflow-x-auto">
                ...
                >
                  ...
                  <template v-if="monthlyProjects.data">
                    <template v-if="monthlyProjects.data.length === 0">
                      <tr>
                    ...
                    <template v-else>
                      <tbody class="bg-blueGray-200">
                        <tr
                          v-for="(bill, index) in monthlyProjects.data"
                          :key="index"
                        >
                          <td
                          ...
                          >
                            {{ bill.project && bill.project.name }}
                          </th>
                          <td
                            class="px-2 text-xs align-middle border-t-0 border-l-0 border-r-0 "
                          >
                            {{ bill.client && bill.client.company_name }}
                          </td>
                          <td
                            class="px-2 text-xs align-middle border-t-0 border-l-0 border-r-0 "
                          >
                            {{ bill.client && bill.pic.name }} -
                            {{ bill.client && bill.pic.position }}
                          </td>
                          <td
                            class="px-2 text-xs align-middle border-t-0 border-l-0 border-r-0 "
                          >
                            {{ bill.client && bill.pic.phone_number }}
                          </td>
                          <td
                            class="px-2 text-xs align-middle border-t-0 border-l-0 border-r-0 "
                          >
                            {{ moment(bill.start_date).format("ll") }}
                          </td>
                          <td
                            class="px-2 text-xs align-middle border-t-0 border-l-0 border-r-0 "
                          >
...
console.log(moment.locale("id", id));
console.log(moment("2021-03-01").format("DD MMMM YYYY"));
const store = useStore();
store.dispatch("project/monthlyProjects");
const monthlyProjects = computed(() => {
  return store.getters["project/getMonthlyProjectsState"];
});
</script>